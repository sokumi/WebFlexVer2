...
- 공통 매핑 예정 파일: `WebFlex.Shared/Common/DbContextModelMapperExtensions.cs`
- 공통 예외 예정 파일: `WebFlex.Shared/Exceptions/WebFlexMessageException.cs`
...
- Controller에서는 저장 API DTO를 `PopulateDTOModel<OpcTag>(values)`로 모델에 자동 매핑합니다.
- `PopulateDTOModel`은 매번 `propertyMap`을 넘기는 방식이 아니라 이름 규칙을 자동 처리해야 합니다.
  - 예: `nodeId -> NODE_ID`
  - 예: `tagName -> TAG_NAME`
  - 예: `dataType -> DATA_TYPE`
  - 예: `isCollectEnabled -> IS_COLLECTENABLED`
  - 예: `saveToDatabase -> SAVE_TO_DATABASE`
  - 예: `showOnDashboard -> SHOW_ON_DASHBOARD`
  - 예: `samplingIntervalMs -> SAMPLINGINTERVALMS`
  - 예: `sortOrder -> SORT_ORDER`
  - 예: `isEnabled -> IsEnabled`
- 자동 매핑은 이름에서 `_`를 제거하고 소문자로 비교하는 방식이면 현재 구조에 맞습니다.
...
`dvc1010.ts`에서는 공통 컴포넌트가 반환한 `WebFlexTagRegisterSaveNode[]`를 `DeviceTagSaveNodeDto[]`로 변환해서 `/device/tag/save`로 보냅니다.

예상 타입:

```ts
type DeviceTagSaveNodeDto = {
    nodeId: string;
    tagName: string;
...
type SaveRequest = {
    deviceId: string;
    nodes: DeviceTagSaveNodeDto[];
};
```

`saveTags`에서는 아래처럼 변환합니다.

```ts
nodes: nodes.map(x => ({
    nodeId: x.nodeId,
    tagName: x.displayName,
...
### DVC1010 등록된 태그 그리드 전체 선택

사용자가 추가 요청한 최신 UI 작업입니다.

- `DVC1010`의 등록된 태그 그리드 체크박스 컬럼에 헤더 전체 선택 체크박스를 추가합니다.
- 현재 로드된 `tagRows` 전체를 대상으로 선택/해제합니다.
- 일부만 선택된 경우 헤더 체크박스는 `indeterminate` 상태여야 합니다.
- 삭제 버튼은 기존 `selectedTagIds`를 그대로 사용하므로 Controller 변경은 필요 없습니다.

수정 위치:

- `WebFlex.UI/Scripts/src/ts/views/device/dvc1010.ts`
- `WebFlex.UI/Scripts/src/css/views/device/dvc1010.css`
...
toggleAllTagSelection(checked: boolean): void {
    if (checked) {
        this.selectedTagIds = this.tagRows
            .map(x => x.id)
            .filter(x => x.length > 0);
    } else {
        this.selectedTagIds = [];
...
updateTagCheckAllState(): void {
    ...
    const totalCount = this.tagRows.length;
...
### DeviceTagController 저장 패턴 방향

사용자가 원하는 Controller 스타일은 아래처럼 액션 내부에 흐름이 보이는 형태입니다.

```csharp
Logger.LogInformation("[DeviceTagController.Save] - 호출됨");

await using var tran = await _db.Database.BeginTransactionAsync();

try {
    var model = _db.PopulateDTOModel<OpcTag>(values);

    // model 기준 검증
    // DB 중복 확인
    // 서버에서만 정해야 하는 값 보정

    _db.Set<OpcTag>().Add(model);
    ...
    await tran.RollbackAsync();
    Logger.LogError(ex.InnerException ?? ex, ex.InnerException?.Message ?? ex.Message);
...
- `try/catch/transaction/log`를 공통 함수 안으로 숨기는 방식은 사용자가 원하지 않았습니다.
- 공통화는 `PopulateDTOModel`, `WebFlexMessageException`, 필요 시 `Success/ErrorData` 정도만 합니다.
- `Success/ErrorData`는 당장은 Controller 내부 private 메서드로 둬도 됩니다.
- 새 프로젝트는 만들지 않습니다.
- NuGet/NPM 추가 설치는 현재 필요 없습니다.
...
- `CreatedAt`
- `UpdatedAt`
- JSON에 없거나 0 이하인 `SORT_ORDER`
- JSON에 없거나 0 이하인 `SAMPLINGINTERVALMS`
...
### DeviceTagController 주의 사항

현재 `DeviceTagController.cs`는 실제 `/device/tag/[action]` 라우트입니다.

기존 파일 하단 DTO 이름이 아직 `TestDeviceTagSaveRequest`, `TestDeviceTagSaveNode`로 남아 있으면 실제 페이지용 이름으로 바꿔야 합니다.

- `DeviceTagSaveRequest`
- `DeviceTagSaveNode`

`CreateSortOrderAsync`는 아래 LINQ 오류가 났으므로 `DefaultIfEmpty().MaxAsync()` 조합을 피해야 합니다.

오류:

```text
The LINQ expression ... DefaultIfEmpty(...) could not be translated.
```
...
여러 태그를 한 번에 저장할 때는 매번 DB max를 조회하지 말고, 저장 시작 전에 `nextSortOrder`를 한 번 구한 뒤 `nextSortOrder++`를 사용합니다.

`CreateTagIdAsync()`도 반복 호출하면 같은 요청 안에서 비효율적일 수 있으므로, `CreateNextTagNoAsync()`로 시작 번호를 한 번 구한 뒤 `TAG{nextTagNo++:0000}` 형태를 권장합니다.
...
- 저장성 API는 가능한 경우 `Logger.LogInformation`, `BeginTransactionAsync`, 명시적 `try/catch`, `RollbackAsync`, `CommitAsync` 흐름이 액션 내부에 보이게 작성합니다.
- 예상 가능한 예외는 `WebFlexMessageException`으로 처리합니다.
- 예기치 않은 예외는 `ex.InnerException ?? ex`를 로그로 남기고 실패 응답을 반환합니다.
