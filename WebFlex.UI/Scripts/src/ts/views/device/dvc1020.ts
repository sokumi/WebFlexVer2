...
export default class Page {
    majorGroups: any[] = [];
    groups: any[] = [];
    selectedMajorGroupId: any = null;
    expandedMajorIds = new Set<any>();
    expandedGroupIds = new Set<any>();
    selectedTagIds = new Set<any>();
    tagMap = new Map<any, any>();
    drawerMode: any = null;
    tagKeywords = new Map<any, any>();
    ...
    async loadTree() {
        try {
            ...
            this.majorGroups.forEach((x: any) => this.expandedMajorIds.add(x.id));
    ...
    renderTree() {
        ...
        const totalTagCount = this.groups.reduce((sum: any, x: any) => sum + Number(x.tagCount ?? 0), 0);
        const noneGroups = this.groups.filter((x: any) => !x.majorGroupId);
        ...
        this.majorGroups.forEach((major: any) => {
            const childGroups = this.groups.filter((x: any) => x.majorGroupId === major.id);
            const majorMatch = String(major.name ?? "").toLowerCase().includes(keyword);
            const childMatch = childGroups.some((x: any) => String(x.name ?? "").toLowerCase().includes(keyword));
            ...
            const tagCount = childGroups.reduce((sum: any, x: any) => sum + Number(x.tagCount ?? 0), 0);

            html += `
                <button type="button"
                        class="wf-group-tree-row ${this.selectedMajorGroupId === major.id ? "is-active" : ""}"
                        data-major-id="${escapeHtml(major.id)}">
                    <span class="wf-group-tree-left">
        ...
        if (noneGroups.length > 0) {
            const tagCount = noneGroups.reduce((sum: any, x: any) => sum + Number(x.tagCount ?? 0), 0);
    ...
    renderGroupTable() {
        ...
        const rows = this.groups.filter((x: any) => {
            if (keyword.length === 0) {
                return true;
            }
            ...
            return String(x.name ?? "").toLowerCase().includes(keyword) ||
                ...
                String(x.description ?? "").toLowerCase().includes(keyword);
        });
    ...
    renderGroupRow(group: any, expanded: any) {
        return `
            <tr>
                <td class="wf-icon-col">
                    <button type="button" class="wf-group-expand-btn" data-toggle-group="${escapeHtml(group.id)}">
                        <i data-lucide="${expanded ? "chevron-down" : "chevron-right"}"></i>
                    </button>
                </td>
                <td>
                    <span class="wf-group-badge">
                        <i data-lucide="folder"></i>
                        ${escapeHtml(group.majorGroupName ?? "미지정")}
                    </span>
                </td>
                <td>
                ...
                <td class="wf-number-col">
                    <span class="wf-tag-count-badge">${group.tagCount ?? 0}</span>
                </td>
                <td class="wf-action-col">
                    <button type="button" class="wf-row-icon-btn" data-edit-group="${escapeHtml(group.id)}" title="수정">
                        <i data-lucide="pencil"></i>
                    </button>
                    <button type="button" class="wf-row-icon-btn is-danger" data-delete-group="${escapeHtml(group.id)}" title="삭제">
                        ...
                        <div class="wf-tag-panel-header">
                            ...
                            <div class="wf-tag-panel-tools">
                                <div class="wf-grid-search wf-tag-search">
                                    ...
                                    <input type="text"
                                           class="form-control"
                                           data-tag-keyword="${escapeHtml(groupId)}"
                                           value="${escapeHtml(this.tagKeywords.get(groupId) ?? "")}"
                                           placeholder="태그 검색..." />
                                </div>
                            </div>
                        </div>
    ...
    bindGroupTableEvents() {
        ...
        $("#groupTableBody").find("[data-edit-group]").on("click", event => {
            ...
            const group = this.groups.find((x: any) => x.id === groupId);
    ...
    bindTagCheckEvents(host: any, groupId: any = null) {
        host.find(".wf-tag-check").on("click", (event: any) => event.stopPropagation());
        host.find(".wf-tag-check").on("change", (event: any) => this.onTagCheckChanged(event));

        if (groupId != null) {
            host.find(".wf-tag-check-all").on("change", (event: any) => this.toggleAllTagsInGroup(event, groupId));
        } else {
            host.find(".wf-tag-check-all").on("change", (event: any) => {
    ...
    toggleAllTagsInGroup(event: any, groupId: any) {
        ...
        const rows = tags.filter((tag: any) => this.isMatchedTagKeyword(tag, keyword));
    ...
    renderTags(groupId: any) {
        ...
        const allChecked = rows.length > 0 && rows.every((tag: any) => this.selectedTagIds.has(tag.id));
        ...
        return `
            <table class="wf-group-inner-table">
                ...
                <tbody>
                    ${rows.map((tag: any, index: any) => this.renderTagRow(tag, index)).join("")}
                </tbody>
            </table>
        `;
    }

    renderTagRow(tag: any, index: any) {
        return `
            <tr>
    ...
    isMatchedTagKeyword(tag: any, keyword: any) {
        if (keyword.length === 0) {
            return true;
        }
    ...
    async selectGroupTags(groupId: any, checked: any) {
        if (!this.tagMap.has(groupId)) {
            await this.loadTags(groupId);
        }
    ...
    renderMajorList() {
        const html = this.majorGroups.map((major: any) => this.renderMajorItem(major)).join("");
        ...
        $("#majorList").find("[data-edit-major]").on("click", event => {
            ...
            const major = this.majorGroups.find((x: any) => x.id === majorId) ?? null;
        ...
        return `
            <div class="wf-major-item">
                <div>
                    ...
                    <p>${escapeHtml(major.description ?? "")}</p>
                </div>
                <div>
                    <button type="button" class="wf-row-icon-btn" data-edit-major="${escapeHtml(major.id)}">
                        <i data-lucide="pencil"></i>
                    </button>
                    <button type="button" class="wf-row-icon-btn is-danger" data-delete-major="${escapeHtml(major.id)}">
                        <i data-lucide="trash-2"></i>
                    </button>
                </div>
            </div>
    ...
    openMoveTags() {
        if (this.selectedTagIds.size === 0) {
            notify.warning("이동할 태그를 선택해 주세요.");
            return;
        }
    ...
    async saveMajor() {
        const request = {
            ID: getValue("#txtEditId"),
            MAJOR_GROUP_NAME: getValue("#txtGroupName"),
            SORT_ORDER: this.readNumber("#numSortOrder"),
            DESCRIPTION: this.getDescription()
        };

        if (request.MAJOR_GROUP_NAME.length === 0) {
            notify.warning("대그룹명을 입력해 주세요.");
            return;
        }
    ...
    async saveGroup() {
        const request = {
            ID: getValue("#txtEditId"),
            MAJOR_GROUP_ID: getValue("#selMajorGroup"),
            GROUP_NAME: getValue("#txtGroupName"),
            SORT_ORDER: this.readNumber("#numSortOrder"),
            DESCRIPTION: this.getDescription()
        };

        if (request.GROUP_NAME.length === 0) {
            notify.warning("중그룹명을 입력해 주세요.");
            return;
        }
    ...
    async moveTags() {
        const request = {
            ...
            GROUP_ID: getValue("#selMoveGroup")
        };

        if (request.GROUP_ID.length === 0) {
            notify.warning("이동할 중그룹을 선택해 주세요.");
            return;
        }
    ...
    async postAndReload(url: any, data: any, errorMessage: any, afterSuccess: any = null) {
        try {
            const result = await api.post({ url, data });

            if (!result.success) {
                notify.warning(result.message ?? "처리 중 오류가 발생했습니다.");
                return;
            }
    ...
    renderGroupSelects() {
        $("#selMajorGroup").html([
            ...
            ...this.majorGroups.map((x: any) => `<option value="${escapeHtml(x.id)}">${escapeHtml(x.name)}</option>`)
        ].join(""));

        $("#selMoveGroup").html(this.groups.map((x: any) => `
            <option value="${escapeHtml(x.id)}">
                ${escapeHtml(x.majorGroupName ?? "미지정")} / ${escapeHtml(x.name)}
            </option>
        `).join(""));
    }
    ...
    getDescription() {
        const description = getValue("#txtDescription");
        const descriptionLong = getValue("#txtDescriptionLong");
