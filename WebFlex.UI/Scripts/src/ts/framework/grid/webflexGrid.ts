...
export class WebFlexGrid<TRow extends object = WebFlexGridRow> {
    ...
    public async addRow(row: TRow, top: boolean = false): Promise<void> {
        await this.table.addRow(row, top);
    }

    public async updateData(rows: TRow[]): Promise<void> {
        await this.table.updateData(rows);
    }

    public async deleteRow(row: TRow): Promise<void> {
    ...
    public setFilter(field: string, type: string, value: unknown): void {
    ...
    public showLoading(message: string = "조회 중입니다..."): void {
    ...
    public selectRowByField<TKey extends keyof TRow>(field: TKey, value: TRow[TKey]): boolean {
    ...
    private bindEvents(options: WebFlexGridOptions<TRow>): void {
